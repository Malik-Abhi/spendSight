...
import { AuthSession, ParsedStatementTransaction, Transaction, User } from '../../models/expense';
import {
  addPersonApi,
  createTransaction,
  deleteAllApi,
...
import { baseCategories, buildCategoryTotals, counterpartyKey, currency, monthKey, monthLabel, transactionFingerprint } from './lib/finance';
...
function SpendSightMark({ size = 74 }: { size?: number }) {
  return <img src="/assets/spendsight-mark.png" alt="SpendSight logo" className="spendsight-mark" style={{ width: size, height: size }} />;
}

function SpendSightLogo() {
  return <img src="/assets/spendsight-logo.png" alt="SpendSight" className="spendsight-logo" />;
}

function Donut({ data, total, label }: { data: Array<{ category: string; total: number; color: string }>; total: number; label: string }) {
  const [active, setActive] = useState<{ category: string; total: number; color: string } | null>(null);
  let offset = 25;
  const radius = 41;
  const circumference = 2 * Math.PI * radius;
  const activeSegment = active ?? data[0] ?? null;
  const activePercent = activeSegment && total > 0 ? Math.round((activeSegment.total / total) * 100) : 0;
  return (
    <div className="donut-wrap">
      <svg viewBox="0 0 100 100" className="donut">
        ...
        {data.map((item, index) => {
          ...
          const selected = active?.category === item.category;
          const segment = (
            <circle
              key={item.category}
              cx="50"
              cy="50"
              r={radius}
              className={selected ? 'donut-segment is-active' : 'donut-segment'}
              style={{ animationDelay: `${index * 90}ms`, transformOrigin: '50% 50%' }}
              stroke={item.color}
      ...
      {activeSegment ? (
        <div className="donut-tooltip">
          <i style={{ background: activeSegment.color }} />
          <span>{activeSegment.category}</span>
          <strong>{currency(activeSegment.total)}</strong>
...
function StatementScreen({
  ...
  const analyze = async () => {
    ...
    try {
      ...
      const uniqueParsed = parsed.filter((item) => {
        ...
        if (existing.has(key) || reviewKeys.has(key)) {
          skipped += 1;
          return false;
        }
      ...
      setMessage(`${uniqueParsed.length} new records found${skipped ? `, ${skipped} duplicates ignored` : ''}. Review before adding.`);
    } catch (error) {
  ...
  const save = async () => {
    ...
    const cleaned = review.map(({ reviewId: _reviewId, categoryConfidence: _confidence, needsCategoryReview: _needsReview, ...item }) => ({
      ...
      const key = transactionFingerprint(item);
      if (existing.has(key) || saveKeys.has(key)) {
        skipped += 1;
        return false;
      }
    ...
    if (!cleaned.length) {
      setReview([]);
      setMessage(`No new records added${skipped ? `; ${skipped} duplicates ignored` : ''}.`);
      return;
    }
    const created = await importTransactions(token, cleaned);
    onImport(created);
    setReview([]);
    setMessage(`${created.length} records added${skipped ? `, ${skipped} duplicates ignored` : ''}.`);
  };
...
function SettingsScreen({
  ...
  const monthlyComparison = useMemo(() => {
    const months = Array.from(new Set(transactions.map((item) => monthKey(item.date)).filter(Boolean))).sort((a, b) => b.localeCompare(a));
    return months.map((month) => {
      const rows = transactions.filter((item) => monthKey(item.date) === month);
      ...
      return { month, income, expense, net, rows: rows.length, incomeWidth: (income / max) * 100, expenseWidth: (expense / max) * 100 };
    });
  }, [transactions]);
  ...
  return (
    <div className="screen fade-in">
      ...
      <button className="settings-row" onClick={() => setComparisonOpen(true)}><BarChart3 /> Income vs expenditure <span>{monthlyComparison.length} months</span><ChevronRight /></button>
      ...
      {comparisonOpen ? (
        <Modal title="Monthly comparison" subtitle="Income vs expenditure" onClose={() => setComparisonOpen(false)}>
          {monthlyComparison.length ? (
            <div className="comparison-list">
              {monthlyComparison.map((item) => (
                <article key={item.month} className="comparison-card">
                  <header><div><strong>{monthLabel(item.month)}</strong><span>{item.rows} records</span></div><em className={item.net >= 0 ? 'positive' : 'negative'}>{item.net >= 0 ? '+' : '-'}{currency(Math.abs(item.net))}</em></header>
                  <div className="comparison-bars">
                    <div><span>Income</span><b>{currency(item.income)}</b><i><em style={{ width: `${item.incomeWidth}%` }} /></i></div>
                    <div><span>Expenditure</span><b>{currency(item.expense)}</b><i><em style={{ width: `${item.expenseWidth}%` }} /></i></div>
                  </div>
                </article>
              ))}
            </div>
          ) : (
            <div className="empty-state glass-card"><strong>No records yet</strong><span>Add income or expenses to compare months.</span></div>
          )}
        </Modal>
      ) : null}
      {peopleOpen ? (
